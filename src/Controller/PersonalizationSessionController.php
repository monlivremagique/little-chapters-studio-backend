...
final class PersonalizationSessionController
{
    ...
    #[RateLimit('session_mutation', 'token')]
    #[Route(
    ...
    public function createSession(Request $request): JsonResponse
    {
        ...
        // bookLocale: 'fr' | 'en' | 'nl' — determines which blueprint drives the entire pipeline.
        // It is mandatory: UI locale, customer locale and book locale are distinct contracts.
        $bookLocale = trim((string) ($payload['bookLocale'] ?? ''));
        if (!in_array($bookLocale, ['fr', 'en', 'nl'], true)) {
            return $this->errorResponse('The "bookLocale" field must be one of: fr, en, nl.', Response::HTTP_BAD_REQUEST);
        }
    ...
    #[RateLimit('session_mutation', 'session')]
    #[Route(
        '/api/personalization/sessions/{id}',
    ...
    #[RateLimit('photo_upload', 'session')]
    #[Route(
    ...
    #[Route(
        '/api/personalization/photos/{photoId}/signed/{token}',
        name: 'app_personalization_photos_signed',
        methods: ['GET'],
        defaults: ['_profiler_collect' => false],
    )]
    public function readPhotoSigned(string $photoId, string $token): Response
    {
        try {
            $verifiedId = $this->signedUrlService->verify($token, 'photo_access');
        } catch (\RuntimeException $exception) {
            return $this->errorResponse($exception->getMessage(), Response::HTTP_FORBIDDEN);
        }

        if ($verifiedId !== $photoId) {
            return $this->errorResponse('Signed URL token does not match the requested photo.', Response::HTTP_FORBIDDEN);
        }
        ...
        try {
            $decryptedContent = $this->personalizationPhotoManager->decryptPhotoContent($photo);
        } catch (\RuntimeException $exception) {
            $this->logger->error('Photo decryption failed', [
                'photo_id' => $photoId,
                'error' => $exception->getMessage(),
            ]);
            return $this->errorResponse('Photo content is unavailable.', Response::HTTP_INTERNAL_SERVER_ERROR);
        }

        $this->logger->info('Personalization photo accessed via signed URL.', [
    ...
    #[RateLimit('session_mutation', 'session')]
    #[Route(
        '/api/personalization/sessions/{id}/erase',
        name: 'app_personalization_sessions_erase',
        methods: ['DELETE'],
        defaults: ['_profiler_collect' => false],
    )]
    public function eraseSessionChildData(string $id, Request $request): JsonResponse
    {
        $session = $this->findSession($id);
        ...
        $photos = $session->getPhotos()->toArray();
        $photoCount = 0;
        ...
        foreach ($photos as $photo) {
            ...
            $this->personalizationPhotoManager->hardPurgePhoto($photo);
            ++$photoCount;
        }
        ...
        $session->saveContent(null, null, [], 0);

        $generationJobs = $this->entityManager->getRepository(PersonalizationGenerationJob::class)->findBy(['session' => $session]);
        foreach ($generationJobs as $job) {
            $this->entityManager->remove($job);
        }

        $this->entityManager->flush();

        $this->logger->info('Child data erased from personalization session (right to erasure).', [
            'session_id' => $session->getId(),
            'photos_purged' => $photoCount,
            'child_name_cleared' => null !== $childName,
            'dedication_cleared' => null !== $dedication,
        ]);
        $this->operationalEventRecorder->record('personalization.child_data_erased', 'info', $session->getId(), null, [
            'photos_purged' => $photoCount,
        ]);

        return new JsonResponse([
            'message' => 'Child data has been erased from this personalization session.',
            'photosPurged' => $photoCount,
        ]);
    }
    ...
    {
        // Pass the session's bookLocale so the correct blueprint drives the pipeline.
        // NULL / missing locale falls back to fr_FR inside getBookBySlug.
        $bookLocale = $session->getResolvedBookLocale();
