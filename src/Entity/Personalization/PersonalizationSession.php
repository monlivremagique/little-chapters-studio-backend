...
{
    private const SUPPORTED_BOOK_LOCALES = ['fr', 'en', 'nl'];
    ...
    /** Language of the book content (fr / en / nl). */
    #[ORM\Column(name: 'book_locale', length: 10, options: ['default' => 'fr'])]
    private string $bookLocale = 'fr';

    /** Whether the order confirmation email was sent. Used to prevent duplicate sends. */
    #[ORM\Column(name: 'order_confirmation_email_sent', options: ['default' => false])]
    private bool $orderConfirmationEmailSent = false;
    ...
    public function getBookLocale(): string
    {
        return $this->bookLocale;
    }

    public function getResolvedBookLocale(): string
    {
        if (!\in_array($this->bookLocale, self::SUPPORTED_BOOK_LOCALES, true)) {
            throw new \DomainException(sprintf('Unsupported book locale "%s".', $this->bookLocale));
        }

        return $this->bookLocale;
    }

    public function isOrderConfirmationEmailSent(): bool
    {
        return $this->orderConfirmationEmailSent;
    }

    public function markOrderConfirmationEmailSent(): void
    {
        $this->orderConfirmationEmailSent = true;
    }

    /** 'fr' | 'en' | 'nl' — maps to fr_FR / en_US / nl_NL for attribute lookups. */
    public function setBookLocale(string $bookLocale): void
    {
        $normalizedLocale = strtolower(trim((string) $bookLocale));
        if (!\in_array($normalizedLocale, self::SUPPORTED_BOOK_LOCALES, true)) {
            throw new \InvalidArgumentException(sprintf('Unsupported book locale "%s".', $bookLocale));
        }

        $this->bookLocale = $normalizedLocale;
    }
