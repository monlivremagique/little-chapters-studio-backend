...
final class PersonalizationPreviewGenerator
{
    ...
    private function processQueuedJob(PersonalizationGenerationJob $job): PersonalizationGenerationJob
    {
        ...
        try {
            ...
            if (null !== $job->getProviderJobId()) {
                $this->logger->warning('Replicate generation started but local persistence only partially completed; resuming from provider job.', [
                    'session_id' => $session->getId(),
                    'generation_job_id' => $job->getId(),
                    'provider_job_id' => $job->getProviderJobId(),
    ...
    private function buildGenerationPlan(PersonalizationSession $session, array $book): array
    {
        ...
        foreach ($bookBlueprint['pages'] as $index => $page) {
            ...
            $compiledTitle = $this->replacePlaceholders((string) ($page['title_template'] ?? ''), $session->getChildName(), $session->getBookLocale(), $session->getExtraFields());
            $compiledText = $this->replacePlaceholders((string) ($page['text_template'] ?? ''), $session->getChildName(), $session->getBookLocale(), $session->getExtraFields());
    ...
    private function startPredictionForCurrentPage(
        ...
        try {
            $job->start($providerJobId, $providerStatus, $responsePayload);
            $started = true;
            $this->syncSessionProgress($session, $job, $generationPlan, $state);
            $this->entityManager->flush();
        } catch (\Throwable $exception) {
            ...
            try {
                ...
                $this->logger->error('Replicate provider job was created but the rescue flush also failed.', [
                    'session_id' => $session->getId(),
                    'generation_job_id' => $job->getId(),
    ...
    private function buildReplicateInput(PersonalizationSession $session, array $book, array $pagePlan): array
    {
        ...
        // Inject hero-reference.png as first input if available (locks hero consistency)
        $heroRefPath = sprintf(
            '%s/resources/book-blueprints/%s/generated-pages/hero-reference.png',
            $this->projectDir,
            $session->getBookSlug(),
        );
        if (is_file($heroRefPath) && is_readable($heroRefPath)) {
            $inputImages[] = $this->buildImageDataUri($heroRefPath);
        }

        $inputImages[] = $this->buildPageReferenceInput($pagePlan);
        $inputImages[] = $this->buildReplicateChildImageInput($session);
    ...
    private function buildPagePrompt(PersonalizationSession $session, array $book, array $pagePlan): string
    {
        ...
        $compiledBookTitle = $this->replacePlaceholders($titleTemplate, $session->getChildName(), $session->getBookLocale(), $session->getExtraFields());
        ...
        $pagePrompt = $this->replacePlaceholders((string) ($pagePlan['promptTemplate'] ?? ''), $session->getChildName(), $session->getBookLocale(), $session->getExtraFields());
    ...
    private function replacePlaceholders(string $template, ?string $childName = null, ?string $bookLocale = null, ?array $extraFields = null): string
    {
        $childName = trim((string) $childName) ?: 'l\'enfant';
        $locale = $bookLocale ?? 'fr';
        $gender = is_array($extraFields) && isset($extraFields['childGender'])
            ? (string) $extraFields['childGender']
            : 'neutral';

        return \App\Personalization\PronounResolver::resolve($template, $childName, $gender, $locale);
    }
