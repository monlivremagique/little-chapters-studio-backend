...
final class CriticalAlertDispatcher
{
    ...
    public function dispatch(string $type, array $context = []): void
    {
        ...
        try {
            ...
            if ('' !== $webhookUrl) {
                $statusCode = $this->httpClient->request('POST', $webhookUrl, [
                    'headers' => [
                ...
                $webhookDelivered = $statusCode >= 200 && $statusCode < 300;

                if (!$webhookDelivered) {
                    $this->logger->warning('critical.alert.webhook_non_success_status', [
                        'alert_type' => $type,
