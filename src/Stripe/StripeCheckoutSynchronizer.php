...
final class StripeCheckoutSynchronizer
{
    ...
    public function handleWebhookEvent(array $event): ?StripeCheckoutSession
    {
        ...
        if (!$checkoutSession instanceof StripeCheckoutSession) {
            ...
            $this->storePendingWebhookEvent($providerEventId, $providerSessionId, $eventType, $payload);
            $this->entityManager->flush();
    ...
    public function replayPendingForProviderSessionId(string $providerSessionId): void
    {
        /** @var list<StripePendingWebhookEvent> $pendingEvents */
        $pendingEvents = $this->entityManager->getRepository(StripePendingWebhookEvent::class)->findBy([
            'providerSessionId' => $providerSessionId,
            'status' => 'pending',
        ], ['id' => 'ASC']);

        foreach ($pendingEvents as $pendingEvent) {
            $this->handleWebhookEvent($pendingEvent->getPayload());
            $pendingEvent->markProcessed();
        }
    ...
    private function applyProviderPayload(
        ...
        if ($paymentStatus === 'paid') {
            $contractErrors = $this->validatePaidProviderPayload($checkoutSession, $providerPayload);
            if ([] !== $contractErrors) {
                $message = sprintf('Stripe paid payload failed validation: %s', implode('; ', $contractErrors));
                $checkoutSession->markFailed($message, $paymentStatus, $paymentIntentId);
            ...
            $this->markPaymentCompleted($checkoutSession, $providerEventId, $providerEventType, $providerPayload);
     ...
     * @return list<string>
     */
    private function validatePaidProviderPayload(StripeCheckoutSession $checkoutSession, array $providerPayload): array
    {
        $errors = [];
        $amountTotal = $providerPayload['amount_total'] ?? null;
        $currency = isset($providerPayload['currency']) ? strtoupper((string) $providerPayload['currency']) : '';
        $metadata = is_array($providerPayload['metadata'] ?? null) ? $providerPayload['metadata'] : [];

        if (!is_numeric($amountTotal) || (int) $amountTotal !== $checkoutSession->getAmountTotal()) {
            $errors[] = sprintf('amount_total expected %d, got %s', $checkoutSession->getAmountTotal(), is_scalar($amountTotal) ? (string) $amountTotal : 'missing');
        }

        if ($currency !== $checkoutSession->getCurrencyCode()) {
            $errors[] = sprintf('currency expected %s, got %s', $checkoutSession->getCurrencyCode(), '' !== $currency ? $currency : 'missing');
        }

        $expectedMetadata = [
            'sylius_order_id' => (string) $checkoutSession->getSyliusOrderId(),
            'sylius_order_number' => $checkoutSession->getSyliusOrderNumber(),
            'sylius_order_token_value' => $checkoutSession->getSyliusOrderTokenValue(),
            'sylius_payment_id' => (string) $checkoutSession->getSyliusPaymentId(),
        ];

        foreach ($expectedMetadata as $key => $expectedValue) {
            if ((string) ($metadata[$key] ?? '') !== $expectedValue) {
                $errors[] = sprintf('metadata.%s mismatch', $key);
            }
        }
    ...
    private function storePendingWebhookEvent(string $providerEventId, string $providerSessionId, string $eventType, array $payload): void
    {
        /** @var StripePendingWebhookEvent|null $existing */
        $existing = $this->entityManager->getRepository(StripePendingWebhookEvent::class)->findOneBy([
            'providerEventId' => $providerEventId,
        ]);

        if ($existing instanceof StripePendingWebhookEvent) {
            return;
        }

        $this->entityManager->persist(new StripePendingWebhookEvent($providerEventId, $providerSessionId, $eventType, $payload));
    }
    ...
    private function markPaymentCompleted(
        ...
        $customerDetails = is_array($providerPayload['customer_details'] ?? null) ? $providerPayload['customer_details'] : [];
        $customerEmail = trim((string) ($customerDetails['email'] ?? $providerPayload['customer_email'] ?? ''));
        if ('' !== $customerEmail) {
            $details['stripe']['customer_email'] = $customerEmail;
        }
