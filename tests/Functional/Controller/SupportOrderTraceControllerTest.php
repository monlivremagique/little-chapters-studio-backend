...
final class SupportOrderTraceControllerTest extends WebTestCase
{
    public function testSupportTraceEndpointReturnsCorrelatedOrderArtifacts(): void
    {
        ...
        $pdfAccessToken = sprintf('trace-pdf-%s', bin2hex(random_bytes(8)));
        $pdf = new PdfArtifact($session, $version, '/tmp/test.pdf', sprintf('/api/personalization/pdfs/%s', $pdfAccessToken), $pdfAccessToken, str_repeat('a', 64), 1234);
        ...
        $entityManager->persist(new StripeCheckoutSession(sprintf('cs_trace_%s', bin2hex(random_bytes(6))), $orderId, $orderNumber, $orderTokenValue, $paymentId, 4990, 'EUR', null, $session->getGuestOwnerToken()));
        $entityManager->flush();
